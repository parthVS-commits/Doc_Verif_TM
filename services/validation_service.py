...
class DocumentValidationService:
    ...
    def validate_documents(
        ...
        """
        Main document validation method with service type detection
        ...
        Returns:
            Tuple[Dict[str, Any], Dict[str, Any]]: Validation results (standard_result, detailed_result)
        """
        start_time = time.time()
        
        # Store preconditions for later use
        ...
        try:
            ...
            # Determine service type and use appropriate validation
            if service_id == "8":  # TM Service
                ...
                is_compliant = all(
                    director.get('is_valid', False) 
                    for director_key, director in directors_validation.items() 
                    if isinstance(director, dict) and director_key not in ['global_errors', 'rule_validations']
                )
    ...
    def _force_service_id_rules(self, all_rules, target_service_id):
        """
        Forcibly select rules for a specific service ID
        
        Args:
            all_rules (list): Retrieved rules
            target_service_id (str): Target service ID
        
        Returns:
            dict: Selected rules for the target service
        """
        self.logger.info(f"Forcing selection of rules for service ID: {target_service_id}")
        
        # Loop through all rule sets to find the matching service ID
        for rule_set in all_rules:
            if isinstance(rule_set, dict) and str(rule_set.get('service_id')) == str(target_service_id):
                self.logger.info(f"Found matching rule set for service ID {target_service_id}")
                return {
        ...
        self.logger.warning(f"No rules found for service ID {target_service_id}, using default rules")
        default_rules = self._get_default_compliance_rules()
    ...
    def _apply_compliance_rules(
        ...
        try:
            ...
            try:
                ...
                # Rule processing map - UPDATED with new GST rules
                rule_processing_map = {
                    # Existing rules
                ...
                for rule in rules:
                    ...
                    try:
                        ...
                        # Special handling for new GST-specific rules
                        elif rule_id in ["AADHAR_PAN_NAME_MATCH", "TENANT_EB_NAME_MATCH", "DOCUMENT_SIGNATURES"]:
                            validation_result = validation_method(directors_validation, conditions)
                        elif rule_id in ["NOC_MULTIPLE_SIGNATURES", "CONSENT_LETTER_VALIDATION", "BOARD_RESOLUTION_VALIDATION"]:
                            validation_result = validation_method(company_docs_validation, conditions)
                        else:
                ...
                self.logger.error(f"Error extracting or processing rules: {e}", exc_info=True)
                return {
                    "global_error": {
                        "status": "error",
                        "error_message": f"Error extracting or processing rules: {str(e)}"
                    }
                }
        ...
        if landlord_signature_required:
            ...
            try:
                ...
                if not has_landlord_signature:
                    return {
                        "status": "failed",
                        "error_message": "NOC document missing required landlord signature"
                    }
    ...
    # Modify the validate_documents method to handle TM services
    ...
    def _validate_certificate(self, cert_url, cert_type, company_name_visible_required, certificate_legible_required):
        """
            ...
            try:
                ...
                if not extracted_data:
                    ...
                    if has_logo:
                        ...
                        brand_names_found = extracted_data.get('brand_names_found', [])
                        extracted_text = extracted_data.get('extracted_text', '').lower()
                        
                        # Check if brand name is found in text
                        brand_name_visible = (
                            brand_name.lower() in extracted_text or
                            brand_name.lower() in [b.lower() for b in brand_names_found]
                        )
                        
                        if logo_visible or brand_name_visible:
                            logo_or_brand_name_found = True
                            
                            # Add the status to the extracted data
                            extracted_data['brand_name_visible'] = brand_name_visible
